...
// 发布类型变化时清空表单，避免不同类型之间残留无关字段
watch(publishType, () => {
  resetForm()
})

// —— 统一的表单数据（一个对象存储所有类型字段）
const form = reactive({
  // 通用
  title: '',
  location: '',
...
// 把 datetime-local 的 "2026-06-30T19:00" 格式化为 "2026-06-30 19:00"
function formatTime(v: string): string {
  return v ? v.replace('T', ' ') : getCurrentTime()
}
